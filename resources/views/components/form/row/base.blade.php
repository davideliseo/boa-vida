...
@if ($fieldMeta['type'] != 'password')
    <div class="form-group row">
        ...
        @elseif ($action == 'create' || $action == 'edit')
            ...
            @elseif ($fieldMeta['type'] == "rut")
                <div class="col-md-6">
                    <x-input.text :is-required="$fieldMeta['is-required']" :key="$key" :item="$item" />
                    <small id="passwordHelpBlock" class="form-text text-muted pt-1 pl-1">
                        Ingresar RUT sin puntos ni dígito verificador.
                    </small>
                </div>
...
@elseif ($fieldMeta['type'] == "password" && $action != 'show')
    <div> <x-input.password /> </div>
